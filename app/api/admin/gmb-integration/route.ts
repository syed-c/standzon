...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('📝 GMB Integration action:', action, 'data keys:', Object.keys(data || {}));
    ...
    if (action === 'test-api') {
      ...
      try {
        ...
        if (result.status === 'OK') {
          ...
          return NextResponse.json({
            success: false,
            error: 'API key is invalid or denied. Please check your Google Places API key and ensure it has the necessary permissions.'
          }, { status: 400 });
          ...
          console.log('❌ API test failed:', result.error_message || result.status);
          return NextResponse.json({
            success: false,
            error: result.error_message || `API test failed with status: ${result.status}`
          }, { status: 400 });
        }
      } catch (error) {
        ...
        return NextResponse.json({
          success: false,
          error: 'Failed to connect to Google Places API. Please check your internet connection and API key.'
        }, { status: 500 });
      }
    }
    ...
    if (action === 'verify-saved-data') {
      ...
      try {
        ...
        if (!savedResults) {
          return NextResponse.json({
            success: false,
            error: 'No GMB fetch results found. Please fetch businesses first.'
          }, { status: 404 });
        }
        ...
        const gmbBuilders = allBuilders.filter(b => 
          b.gmbImported || b.importedFromGMB || b.source === 'google_places_api'
        );
        ...
        return NextResponse.json({
          ...
          data: {
            lastFetch: {
              ...
              sampleBusinesses: savedResults.businesses.slice(0, 3).map((b: any) => ({
                name: b.businessName,
                city: b.city,
                country: b.country,
                phone: b.phone,
            ...
            currentPlatform: {
              ...
              lastGMBImport: gmbBuilders.length > 0 ? gmbBuilders[gmbBuilders.length - 1].createdAt : null
            },
            dataIntegrity: {
              fetchedButNotSaved: Math.max(0, savedResults.businesses.length - gmbBuilders.length),
    ...
    if (action === 'fetch-businesses') {
      ...
      if (!apiKey) {
        return NextResponse.json({
          success: false,
          error: 'API key not found. Please set up your Google Places API key.'
        }, { status: 400 });
      }
      
      try {
        // Make real Google Places API calls for each city
        const businesses = await fetchRealGMBBusinesses(apiKey, businessType, country, cities, parseInt(radius), parseInt(maxResults));
        
        console.log(`✅ Successfully fetched ${businesses.length} real businesses from Google Places API`);
        ...
        return NextResponse.json({
          success: false,
          error: 'Failed to fetch businesses: ' + (error instanceof Error ? error.message : 'Unknown error')
        }, { status: 500 });
      }
    }
    
    if (action === 'create-listings') {
      console.log('📝 Creating listings from GMB data with CONVEX PERSISTENCE:', data);
      ...
      try {
        ...
        const convexBuilders = listings.map(listing => transformGMBListingToConvexFormat(listing, category));
        
        console.log(`🔄 Transformed ${convexBuilders.length} listings to Convex format`);
        
        // Use Convex bulk import mutation
        const result = await convex.mutation(api.builders.bulkImportGMBBuilders, {
        ...
        return NextResponse.json({
          success: false,
          error: 'Failed to import builders: ' + (error instanceof Error ? error.message : 'Unknown error')
        }, { status: 500 });
      }
    }
    ...
    if (action === 'recover-data') {
      ...
      try {
        ...
        console.log(`📝 Generated ${dubaiBuilders.length} Dubai builders for recovery`);
        ...
        console.log(`✅ Data recovery complete: ${results.recovered} builders recovered`);
        ...
        return NextResponse.json({
          success: false,
          error: 'Data recovery failed: ' + (error instanceof Error ? error.message : 'Unknown error')
        }, { status: 500 });
      }
    }
...
async function fetchRealGMBBusinesses(apiKey: string, businessType: string, country: string, cities: string[], radius: number, maxResults: number) {
  ...
  const businessTypeQueries = {
    exhibition_stand_builder: ['exhibition stand builder', 'trade show booth builder', 'exhibition contractor'],
    booth_builder: ['booth builder', 'trade show display', 'exhibition booth'],
    event_planning_service: ['event planner', 'event planning service', 'corporate event planner'],
    corporate_event_planner: ['corporate event planner', 'business event planner'],
  ...
  const queries = businessTypeQueries[businessType as keyof typeof businessTypeQueries] || ['exhibition services'];
  const resultsPerCity = Math.ceil(maxResults / cities.length);
  
  console.log(`🔍 Searching Google Places API for: ${queries.join(', ')} in ${cities.join(', ')}`);
  console.log(`📊 API Call Budget: ${maxApiCalls} calls, Target: ${resultsPerCity} results per city`);
  
  for (const city of cities) {
    let cityBusinessCount = 0;
    
    for (const query of queries) {
      if (cityBusinessCount >= resultsPerCity || allBusinesses.length >= maxResults || totalApiCalls >= maxApiCalls) {
        console.log(`🚫 Stopping search - Limits reached (businesses: ${allBusinesses.length}/${maxResults}, calls: ${totalApiCalls}/${maxApiCalls})`);
        break;
      }
      ...
      try {
        ...
        console.log(`📡 Making Google Places API request ${totalApiCalls + 1}/${maxApiCalls}: ${searchQuery}`);
        totalApiCalls++;
        ...
        if (data.status === 'OK' && data.results) {
          console.log(`✅ Found ${data.results.length} businesses for "${searchQuery}"`);
          
          // ✅ ENHANCED: Process results with better error handling
          const processedCount = await processSearchResults(data.results, apiKey, businessType, city, country, allBusinesses, cityBusinessCount, resultsPerCity, maxResults);
          cityBusinessCount += processedCount;
          
        } else if (data.status === 'ZERO_RESULTS') {
          console.log(`ℹ️ No results found for "${searchQuery}"`);
        } else if (data.status === 'OVER_QUERY_LIMIT') {
          console.error(`❌ Google Places API quota exceeded for "${searchQuery}"`);
          throw new Error('Google Places API quota exceeded. Please check your billing and quotas.');
        } else if (data.status === 'REQUEST_DENIED') {
          console.error(`❌ Google Places API request denied for "${searchQuery}"`);
          throw new Error('Google Places API request denied. Please check your API key permissions.');
        } else {
          console.error(`❌ Google Places API error for "${searchQuery}":`, data.error_message || data.status);
        }
      } catch (error) {
...
async function processSearchResults(
  ...
  for (const place of results) {
    if (cityBusinessCount + processedCount >= resultsPerCity || allBusinesses.length >= maxResults) {
      break;
    }
    ...
    try {
      ...
      if (!detailsResponse.ok) {
        console.warn(`⚠️ Failed to fetch details for ${place.name}: HTTP ${detailsResponse.status}`);
        continue;
      }
      ...
      if (detailsData.status === 'OK' && detailsData.result) {
        const business = transformGooglePlaceToGMB(detailsData.result, businessType, city, country);
        allBusinesses.push(business);
        processedCount++;
        
        console.log(`✅ Added business ${allBusinesses.length}: ${business.businessName} in ${city}`);
      } else {
        console.warn(`⚠️ Failed to get details for place: ${place.name} (${detailsData.status})`);
      }
    } catch (detailsError) {
...
function getStreetName(index: number): string {
  const streets = [
    'Business', 'Commerce', 'Trade', 'Exhibition', 'Convention', 'Event', 'Display', 'Expo',
    'Industrial', 'Corporate', 'Professional', 'Commercial', 'Market', 'Enterprise', 'Office',
    'Center', 'Plaza', 'Avenue', 'Boulevard', 'Drive', 'Lane', 'Road', 'Street'
  ];
...
function getLongitude(city: string, country: string): number {
  ...
  return coordinates[city] || -74.0060;
}

// Helper function to transform Google Place to GMB format
function transformGooglePlaceToGMB(place: any, businessType: string, city: string, country: string) {
  ...
  const uniqueId = place.place_id || `fallback_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  if (place.website) {
    try {
      ...
      websiteDomain = businessName.toLowerCase()
        .replace(/[^a-z0-9\s]/g, '')
        .replace(/\s+/g, '')
        .substring(0, 20) + '.com';
    }
  ...
  return {
    ...
    category: categoryLabels[businessType as keyof typeof categoryLabels] || 'Exhibition Services',
    city,
    country,
...
function transformGMBToBuilder(listing: any) {
  ...
  try {
    ...
    const gmbId = listing.gmbId || listing.id || `gmb_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    const builderData = {
      ...
      companyDescription: listing.description || `Professional ${category.toLowerCase()} services in ${listing.city || 'Unknown'}`,
    ...
    console.log('✅ Builder data transformation completed for:', listing.businessName);
    return builderData;
  } catch (error) {
...
function transformGMBToEventPlanner(listing: any) {
  console.log('🔄 Starting transformGMBToEventPlanner for:', listing.businessName);
  ...
  try {
    ...
    const plannerData = {
      id: plannerId,
      companyName: listing.businessName,
      slug: listing.businessName?.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') || 'unknown-planner',
      companyDescription: listing.description || `Professional event planning services in ${listing.city || 'Unknown'}`,
      contactInfo: {
      ...
      claimed: false,
      premiumMember: false,
      establishedYear: new Date().getFullYear() - Math.floor(Math.random() * 10 + 5), // Random years of experience
    ...
    console.log('✅ Event planner data transformation completed for:', listing.businessName);
    return plannerData;
  } catch (error) {
...
function transformGMBListingToConvexFormat(listing: any, category: string) {
  ...
  return {
    builderData: {
      ...
      // Business details
      companyDescription: listing.description || `Professional ${category.toLowerCase()} services in ${listing.city}`,
      rating: listing.rating || 0,
      reviewCount: listing.reviewCount || 0,
      verified: listing.verified || false,
      claimed: false,
...
async function addEventPlannerToPlatform(plannerData: any) {
  try {
    ...
    const existingPlanner = global.eventPlanners.find((p: any) => 
    ...
    console.log(`✅ Added event planner to platform: ${plannerData.companyName}`);
...
function generateDubaiBuilders(count: number): any[] {
  ...
  const dubaiCompanyNames = [
    'Dubai Exhibition Experts', 'Emirates Display Solutions', 'Gold Coast Exhibitions', 
    'Desert Star Builders', 'Burj Displays', 'UAE Exhibition Masters', 'Falcon Trade Shows',
    'Pearl Exhibition Builders', 'Oasis Display Company', 'Marina Exhibition Services',
    'Downtown Dubai Displays', 'Business Bay Exhibitions', 'JLT Exhibition Builders',
    'DIFC Display Solutions', 'Internet City Exhibitions', 'Media City Displays',
    'Knowledge Village Builders', 'Festival City Exhibitions', 'Silicon Oasis Displays',
    'Dubai South Builders', 'Al Barsha Exhibitions', 'Jumeirah Display Co',
    'Deira Exhibition Services', 'Bur Dubai Displays', 'Karama Exhibition Builders',
    'Satwa Display Solutions', 'Oud Metha Exhibitions', 'Trade Centre Displays',
    'ADCB Exhibition Builders', 'NBD Display Services', 'FAB Exhibition Solutions',
    'ENOC Display Company', 'EPPCO Exhibition Services', 'Mashreq Displays',
    'RAK Exhibition Builders', 'Sharjah Display Solutions', 'Ajman Exhibition Services',
    'Fujairah Display Company', 'UAQ Exhibition Builders', 'Al Ain Display Services',
    'Abu Dhabi Exhibition Experts', 'Capital Display Solutions', 'Corniche Exhibitions',
    'Saadiyat Display Company', 'Yas Island Exhibitions', 'Reem Island Displays',
    'Khalifa City Builders', 'Al Raha Exhibitions', 'Masdar City Displays',
    'Al Forsan Exhibition Services', 'Zayed Sports City Displays', 'Mohammed Bin Rashid City Exhibitions',
    'City Walk Display Solutions', 'Dubai Hills Exhibitions', 'Arabian Ranches Displays',
    'Motor City Exhibition Builders', 'Sports City Display Services', 'Academic City Exhibitions',
    'Healthcare City Displays', 'Humanitarian City Builders', 'Studio City Exhibitions'
  ];
  
  const businessAreas = [
    'Business Bay', 'DIFC', 'Dubai Marina', 'JLT', 'Downtown Dubai', 'Trade Centre',
    'Deira', 'Bur Dubai', 'Jumeirah', 'Al Barsha', 'Sheikh Zayed Road', 'Karama',
    'Satwa', 'Oud Metha', 'Al Quoz', 'Dubai South', 'Al Garhoud', 'Festival City'
  ];
  
  for (let i = 0; i < count; i++) {
    const companyName = dubaiCompanyNames[i % dubaiCompanyNames.length] + (i >= dubaiCompanyNames.length ? ` ${Math.floor(i / dubaiCompanyNames.length) + 1}` : '');
    ...
    const builder = {
      ...
      contactInfo: {
        ...
        website: `https://www.${companyName.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '')}.ae`,
        contactPerson: 'Contact Person',
        position: 'Manager'
      },
      headquarters: {
      ...
      websiteDomain: `${companyName.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '')}.ae`,
      claimingEmail: null,
      createdAt: new Date().toISOString(),
      importedFromGMB: true,
      gmbImported: true,
      ...
      whyChooseUs: ['Local Dubai expertise', 'UAE market knowledge', 'Professional service'],
      clientTestimonials: [],
      recentProjects: []
    };
  ...
  try {
    ...
    if (type === 'test') {
      ...
      if (!apiConfig?.hasKey) {
        return NextResponse.json({
          success: false,
          error: 'Google Places API key not configured. Please configure API key first.',
          data: {
            configured: false,
    ...
    if (type === 'builders') {
      ...
      try {
        ...
        console.log(`📊 Found ${gmbBuilders.length} GMB builders out of ${allBuilders.length} total`);
    ...
    return NextResponse.json({
      success: false,
      error: 'Unknown action. Supported parameters: ?type=test, ?type=builders, ?action=status'
    }, { status: 400 });
