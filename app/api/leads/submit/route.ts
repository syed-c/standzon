...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const leadData = {
      ...
      contactPerson: body.contactName || body.name || body.contactPerson || 'Not specified',
      ...
      exhibitionSlug: body.exhibitionSlug || (body.exhibitionName ? 
        body.exhibitionName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') : undefined),
      ...
      countryName: body.country || body.countryName || body.builderLocation?.split(',').pop()?.trim(),
      cityName: body.city || body.cityName || body.location || body.builderLocation?.split(',')[0]?.trim(),
      venue: body.venue,
      
      // Requirements and preferences
      services: body.services ? (Array.isArray(body.services) ? body.services : [body.services]) : undefined,
      specialRequirements: body.message || body.requirements || body.specialRequirements,
      designPreferences: body.designPreferences,
      
      // Lead management
      status: body.status || 'new',
      priority: body.priority || body.urgency || (body.timeline?.includes('1-2 months') ? 'high' : 'medium'),
    ...
    let allBuilders = [];
    if (buildersResult && buildersResult.ok && Array.isArray(buildersResult.builders)) {
    ...
    const qualifiedBuilders = allBuilders.filter(builder => {
      ...
      const locationMatch = builderLocation.includes(leadCity) || 
      ...
      const serviceLocationMatch = builder.serviceLocations?.some(loc => 
    ...
    try {
      ...
      const buildersWithCredits = qualifiedBuilders.filter(builder => {
        ...
        const hasCredits = builder.subscriptionPlan !== 'free' || (builder.leadCredits && builder.leadCredits > 0);
        const isActive = builder.isActive !== false;
        return hasCredits && isActive;
      });
      
      console.log(`📧 Sending notifications to ${buildersWithCredits.length} builders with credits`);
      
      // Send notifications to qualified builders
      for (const builder of buildersWithCredits.slice(0, 5)) { // Limit to top 5 builders
        try {
          ...
          console.log(`📧 Notifying builder: ${builder.companyName} (${builder.contactEmail || builder.email})`);
          ...
          console.error(`❌ Failed to notify builder ${builder.companyName}:`, notificationError);
        }
      }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        qualifiedBuilders: qualifiedBuilders.slice(0, 5).map(b => ({
          ...
          location: b.location,
          hasCredits: b.subscriptionPlan !== 'free' || (b.leadCredits && b.leadCredits > 0)
        })),
        notificationsSent,
        emailsSent,
        routingSuccess: true,
