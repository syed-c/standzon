...
import { FiCalendar, FiMapPin, FiUsers, FiTrendingUp, FiSearch, FiFilter, FiArrowRight, FiGlobe, FiStar, FiEye } from 'react-icons/fi';
import { tradeShows, industries, tradeShowStats, TradeShowUtils } from '@/lib/data/tradeShows';
import { exhibitions as comprehensiveExhibitions } from '@/lib/data/exhibitions';

export default function TradeShowsDirectory() {
  console.log("Trade Shows Directory: Page loaded with database", tradeShowStats);
  console.log("Comprehensive Exhibitions loaded:", comprehensiveExhibitions.length);
  ...
  const allExhibitions = useMemo(() => {
    ...
    const convertedExhibitions = comprehensiveExhibitions.map(exhibition => ({
      ...
      previousEditionStats: exhibition.previousEditions?.length > 0 ? {
    ...
    const uniqueExhibitions = combined.filter((exhibition, index, self) => 
      index === self.findIndex(e => e.id === exhibition.id)
    );

    console.log(`Combined exhibitions: ${tradeShows.length} trade shows + ${convertedExhibitions.length} comprehensive exhibitions = ${uniqueExhibitions.length} total unique exhibitions`);
  ...
  const countries = useMemo(() => {
    return Array.from(new Set(allExhibitions.map(show => show.country))).sort();
  }, [allExhibitions]);
  ...
  const enhancedStats = useMemo(() => {
    ...
    const totalExpectedVisitors = allExhibitions.reduce((sum, show) => sum + show.expectedVisitors, 0);
    const totalExpectedExhibitors = allExhibitions.reduce((sum, show) => sum + show.expectedExhibitors, 0);
    
    return {
      totalShows,
      totalCountries,
      totalCities: Array.from(new Set(allExhibitions.map(show => show.city))).length,
      totalIndustries,
      ...
      regionalCount: allExhibitions.filter(s => s.significance === 'Regional').length,
      countryBreakdown: countries.reduce((acc, country) => {
        acc[country] = allExhibitions.filter(s => s.country === country).length;
        return acc;
  ...
  return (
    <div className="font-inter min-h-screen bg-background-gray" data-macaly="trade-shows-main-container">
      <Navigation />
      ...
      <section className="pt-20 pb-16 bg-gradient-to-br from-deep-navy via-gray-800 to-primary-blue text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-4xl mx-auto" data-macaly="hero-description">
              Access {enhancedStats.totalShows}+ verified trade shows across {enhancedStats.totalCountries} countries. 
              Connect with exhibition stand builders and maximize your ROI at industry-leading events.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-lg mb-8">
              ...
              <div className="flex flex-col items-center space-y-2">
                ...
                <span className="font-semibold">{(enhancedStats.totalExpectedVisitors / 1000000).toFixed(1)}M+</span>
      ...
      <section className="py-8 bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-3 md:grid-cols-6 gap-6 text-center">
            ...
            <div>
              <div className="text-3xl font-bold text-red-600">{(enhancedStats.totalExpectedVisitors / 1000000).toFixed(1)}M</div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="text-gray-600 font-medium">
                <span className="text-primary-blue font-bold">{filteredShows.length}</span> of {allExhibitions.length} exhibitions
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-4">
                <Select value={selectedCountry} onValueChange={setSelectedCountry}>
                  ...
                  <SelectContent>
                    ...
                    {countries.map(country => {
                      const count = enhancedStats.countryBreakdown[country] || 0;
                      return (
                ...
                <Select value={selectedIndustry} onValueChange={setSelectedIndustry}>
                  ...
                  <SelectContent>
                    ...
                    {industries.map(industry => {
                      const count = allExhibitions.filter(show => 
                        show.industries && show.industries.some(ind => ind.slug === industry.slug)
                      ).length;
                      return (
                ...
                <Select value={selectedYear} onValueChange={setSelectedYear}>
                  ...
                  <SelectContent>
                    ...
                    {years.map(year => {
                      const count = allExhibitions.filter(s => s.year === year).length;
                      return (
        ...
        <section className="py-16 bg-background-gray">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-deep-navy mb-4" data-macaly="major-shows-title">
                Major International Trade Shows
              </h2>
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Industry-leading exhibitions featuring global audiences, major exhibitors, and exceptional networking opportunities. 
                Featuring {majorShows.length} major events across {enhancedStats.totalCountries} countries.
              </p>
            </div>
                  ...
                  <Card className="hover:shadow-xl transition-all duration-300 group-hover:-translate-y-2 border-0 shadow-lg">
                    <CardHeader className="pb-4">
                      <div className="flex items-center justify-between mb-3">
                        <Badge 
                          variant="secondary" 
                          style={{ backgroundColor: show.industries[0]?.color + '15', color: show.industries[0]?.color, border: `1px solid ${show.industries[0]?.color}30` }}
                          className="font-medium"
                        >
                          {show.industries[0]?.icon} {show.industries[0]?.name}
                        </Badge>
                        <div className="flex gap-1">
                          <Badge variant="outline" className="bg-warning-orange/10 text-warning-orange border-warning-orange/30">
                            {show.significance}
                          </Badge>
                          {new Date(show.startDate) > new Date() && (
                            <Badge className="bg-industry-green text-white">Upcoming</Badge>
                          )}
                        </div>
                      </div>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span className="font-medium">{show.city}, {show.country}</span>
                        </div>
                        <div className="flex items-center text-sm text-gray-600">
                          <FiCalendar className="w-4 h-4 mr-3 text-primary-blue" />
                          <span>{TradeShowUtils.formatDate(show.startDate)} - {TradeShowUtils.formatDate(show.endDate)}</span>
                        </div>
                        <div className="flex items-center text-sm text-gray-600">
                          <FiUsers className="w-4 h-4 mr-3 text-primary-blue" />
                          <span className="font-medium">{show.expectedVisitors.toLocaleString()}</span> expected visitors
                        </div>
                        <div className="flex items-center text-sm text-gray-600">
                          <FiEye className="w-4 h-4 mr-3 text-primary-blue" />
                          <span className="font-medium">{show.expectedExhibitors.toLocaleString()}</span> exhibitors
                        </div>
                        ...
                        {show.previousEditionStats && (
                          <div className="flex items-center text-sm text-gray-600 bg-gray-50 p-2 rounded">
                            ...
                            <span>Previous edition: <span className="font-medium">{show.previousEditionStats.feedback}/5</span> rating</span>
                          </div>
                        )}
                        <div className="pt-3">
          ...
          <div className="text-center mb-12">
            ...
            {filteredShows.length > 0 && (
              <p className="text-lg text-gray-600">
                Comprehensive exhibition database with detailed information, costs, and builder recommendations.
                {selectedCountry !== 'all' && ` Showing events in ${selectedCountry}.`}
                {selectedIndustry !== 'all' && ` Filtered by ${industries.find(i => i.slug === selectedIndustry)?.name}.`}
              </p>
            )}
          </div>
          ...
          {filteredShows.length > 0 ? (
                      ...
                      <Card className="h-full hover:shadow-lg transition-all duration-300 group-hover:-translate-y-1 border border-gray-200">
                        <CardHeader className="pb-3">
                          <div className="flex items-center justify-between mb-2">
                            <Badge 
                              variant="secondary" 
                              style={{ backgroundColor: show.industries[0]?.color + '15', color: show.industries[0]?.color }}
                        ...
                        <CardContent>
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center text-sm text-gray-600">
                              ...
                              {TradeShowUtils.formatDate(show.startDate)} ({duration}d)
                            </div>
                            <div className="flex items-center text-sm text-gray-600">
                              <FiUsers className="w-3 h-3 mr-2 text-primary-blue" />
                              {(show.expectedVisitors / 1000).toFixed(0)}K visitors
                            </div>
                            <div className="flex items-center text-sm text-gray-600">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-6 max-w-md mx-auto">
                Try adjusting your search criteria or filters to find relevant trade shows. 
        ...
        <section className="py-16 bg-background-gray">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <p className="text-lg text-gray-600 max-w-2xl mx-auto">
                Explore exhibitions across {industries.length} major industry sectors, featuring {enhancedStats.totalShows} 
                international trade shows with detailed market insights and growth analytics.
              </p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {industries.map((industry) => {
                const industryShows = allExhibitions.filter(show => 
                  show.industries && show.industries.some(ind => ind.slug === industry.slug)
                );
                const majorIndustryShows = industryShows.filter(s => s.significance === 'Major').length;
                return (
                  <Card key={industry.id} className="hover:shadow-xl transition-all duration-300 cursor-pointer border-0 shadow-lg hover:-translate-y-1"
                    ...
                    <CardHeader className="pb-4">
                      <CardTitle className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="text-lg font-bold text-deep-navy">{industry.name}</div>
                          <div className="text-sm text-gray-600 font-medium">
                            {industryShows.length} exhibitions • {majorIndustryShows} major events
                          </div>
                        </div>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded">
                          ...
                          <span className="font-semibold text-industry-green">+{industry.annualGrowthRate}%</span>
                        </div>
                        <div className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded">
                          <span className="text-gray-600">Avg. Booth Cost:</span>
                          <span className="font-semibold text-deep-navy">€{industry.averageBoothCost}/m²</span>
                        </div>
                        <div className="flex items-center justify-between text-sm bg-gray-50 p-2 rounded">
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600">
              Real-time data and analytics from the world's largest trade show database
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <Card className="text-center border-0 shadow-lg">
              <CardContent className="pt-6">
                ...
                <div className="text-gray-600 font-medium">Total Expected Visitors</div>
                <div className="text-xs text-gray-500 mt-1">Across all exhibitions</div>
              </CardContent>
            </Card>
            <Card className="text-center border-0 shadow-lg">
              <CardContent className="pt-6">
                ...
                <div className="text-gray-600 font-medium">Total Exhibitors</div>
                <div className="text-xs text-gray-500 mt-1">Global participation</div>
              </CardContent>
            </Card>
            <Card className="text-center border-0 shadow-lg">
              <CardContent className="pt-6">
                ...
                <div className="text-gray-600 font-medium">Average Show Size</div>
                <div className="text-xs text-gray-500 mt-1">Visitors per event</div>
              </CardContent>
            </Card>
            <Card className="text-center border-0 shadow-lg">
              <CardContent className="pt-6">
      ...
      <section className="py-16 bg-gradient-to-br from-deep-navy via-gray-800 to-primary-blue text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl mb-8 text-gray-300 max-w-2xl mx-auto">
            Access our vetted network of experienced exhibition contractors who specialize in your target shows. 
            Get competitive quotes, expert guidance, and maximize your exhibition ROI.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact">
              <Button size="lg" className="bg-primary-blue hover:bg-blue-dark text-white px-8 py-4 text-lg">
                <FiUsers className="w-5 h-5 mr-2" />
                Find Exhibition Builders
              </Button>
            </Link>
            <Link href="/exhibition-stands">
              <Button size="lg" variant="outline" className="border-white text-white hover:bg-white hover:text-deep-navy px-8 py-4 text-lg">
          ...
          <div className="mt-8 text-sm text-gray-400">
            Trusted by 10,000+ exhibitors worldwide • 98% customer satisfaction rate
          </div>
        </div>
      </section>
