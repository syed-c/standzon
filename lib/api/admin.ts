...
export const adminAPI = {
  ...
  async getStats(): Promise<{ success: boolean; data: AdminStats; error?: string }> {
    try {
      ...
      const countries = Array.from(new Set(builders.map(b => b.headquarters?.country).filter(Boolean)));
      const cities = Array.from(new Set(builders.flatMap(b => (b.serviceLocations || []).map(l => l.city).filter(Boolean))));
      const verifiedBuilders = builders.filter(b => b.verified).length;
      const totalRating = builders.reduce((sum, b) => sum + (b.rating || 0), 0);
      const averageRating = builders.length > 0 ? totalRating / builders.length : 0;
  ...
  async getBuilders(page: number = 1, limit: number = 50, filters?: any): Promise<{ 
    success: boolean; 
    data?: any[]; 
  ...
  async getBuilder(id: string): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
  ...
  async createBuilder(builderData: any): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
  ...
  async updateBuilder(id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
  ...
  async deleteBuilder(id: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  clearAllData: async (): Promise<{ success: boolean; message: string; error?: string }> => {
    try {
  ...
  bulkCreateBuilders: async (builders: any[]): Promise<{ success: boolean; data?: any; error?: string }> => {
    try {
  ...
  async getCountries(): Promise<{ success: boolean; data?: any[]; error?: string }> {
    try {
      const builders = await unifiedPlatformAPI.getBuilders();
  ...
  async getCities(): Promise<{ success: boolean; data?: any[]; error?: string }> {
    try {
      const builders = await unifiedPlatformAPI.getBuilders();
      ...
      const buildersByCity = builders.reduce((acc: any, builder: any) => {
        ...
        const locations = [builder.headquarters, ...(builder.serviceLocations || [])];
  ...
  async createCountry(countryData: any): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      const newCountry = {
        ...
        seoData: {
          ...
          keywords: [`${countryData.name} exhibitions`, 'trade shows', 'exhibition stands']
        }
      };
      ...
      console.log(`✅ Country added: ${countryData.name} with ${countryData.majorCities?.length || 0} cities`);
  ...
  async createCity(cityData: any): Promise<{ success: boolean; data?: any; error?: string }> {
    try {
      // Find the associated country or create default values
      const country = GLOBAL_EXHIBITION_DATA.countries.find(c => 
        ...
        slug: (cityData.country || 'unknown').toLowerCase().replace(/\s+/g, '-')
      };
      ...
      const newCity = {
        ...
        countryCode: (country as any).countryCode || (country as any).code || 'XX',
        continent: country.continent,
        ...
        keyIndustries: cityData.keyIndustries || ['Technology', 'Manufacturing'],
        ...
        seoData: {
          ...
          keywords: [`exhibition stands ${cityData.name}`, `trade show builders ${country.name}`, `booth construction`]
        }
      };
  ...
  async updateCountry(id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> {
  ...
  async updateCity(id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> {
    // Mock implementation for compatibility
    return {
      success: true,
      data: { id, ...updates }
  ...
  async deleteCountry(id: string): Promise<{ success: boolean; error?: string }> {
    // Mock implementation for compatibility
    return { success: true };
  },
  ...
  async getExhibitions(page: number = 1, limit: number = 50, filters?: any): Promise<{ success: boolean; data?: any[]; error?: string }> {
    try {
      // Mock implementation
      return {
        success: true,
  ...
  async createExhibition(exhibitionData: any): Promise<{ success: boolean; data?: any; error?: string }> {
    // Mock implementation for compatibility
    return {
      success: true,
  ...
  async updateExhibition(id: string, updates: any): Promise<{ success: boolean; data?: any; error?: string }> {
    // Mock implementation for compatibility
    return {
      success: true,
      data: { id, ...updates }
  ...
  async deleteExhibition(id: string): Promise<{ success: boolean; error?: string }> {
    // Mock implementation for compatibility
    return { success: true };
  },

  // Intelligence methods (mock implementations for compatibility)
  async getPlatformIntelligence(): Promise<{ success: boolean; data?: any; error?: string }> {
    return {
      success: true,
      data: {
        dataPoints: 1847,
  ...
  async getEventIntelligence(): Promise<{ success: boolean; data?: any; error?: string }> {
    return {
      success: true,
      data: {
        upcomingEvents: 24,
        trends: ['Virtual hybrid events', 'Sustainable materials', 'Interactive displays']
      }
    };
  },

  // Helper function to populate missing countries and cities from user's comprehensive list
  async populateComprehensiveLocations(): Promise<{ success: boolean; added: { countries: number; cities: number }; error?: string }> {
    try {
      ...
      const requiredData = [
        { country: 'United Arab Emirates', code: 'AE', cities: ['Dubai', 'Abu Dhabi', 'Sharjah', 'Ras Al Khaimah'], continent: 'Asia' },
        { country: 'India', code: 'IN', cities: ['New Delhi', 'Mumbai', 'Bangalore', 'Chennai', 'Hyderabad', 'Ahmedabad', 'Lucknow', 'Kanpur'], continent: 'Asia' },
        { country: 'Qatar', code: 'QA', cities: ['Doha'], continent: 'Asia' },
        { country: 'Oman', code: 'OM', cities: ['Muscat'], continent: 'Asia' },
        { country: 'Saudi Arabia', code: 'SA', cities: ['Riyadh', 'Jeddah', 'Dammam'], continent: 'Asia' },
        { country: 'Bahrain', code: 'BH', cities: ['Manama'], continent: 'Asia' },
        { country: 'Kuwait', code: 'KW', cities: ['Kuwait City'], continent: 'Asia' },
        { country: 'Malaysia', code: 'MY', cities: ['Kuala Lumpur', 'Penang'], continent: 'Asia' },
        { country: 'Singapore', code: 'SG', cities: ['Singapore'], continent: 'Asia' },
        { country: 'Indonesia', code: 'ID', cities: ['Jakarta'], continent: 'Asia' },
        { country: 'Vietnam', code: 'VN', cities: ['Ho Chi Minh City'], continent: 'Asia' },
        { country: 'Philippines', code: 'PH', cities: ['Manila'], continent: 'Asia' },
        { country: 'China', code: 'CN', cities: ['Shanghai', 'Beijing', 'Guangzhou', 'Shenzhen'], continent: 'Asia' },
        { country: 'Japan', code: 'JP', cities: ['Tokyo', 'Osaka'], continent: 'Asia' },
        { country: 'South Korea', code: 'KR', cities: ['Seoul', 'Busan'], continent: 'Asia' },
        { country: 'Germany', code: 'DE', cities: ['Frankfurt', 'Munich', 'Berlin', 'Cologne', 'Düsseldorf', 'Hannover'], continent: 'Europe' },
        { country: 'France', code: 'FR', cities: ['Paris', 'Lyon', 'Marseille'], continent: 'Europe' },
        { country: 'Italy', code: 'IT', cities: ['Milan', 'Rome', 'Bologna'], continent: 'Europe' },
        { country: 'United Kingdom', code: 'GB', cities: ['London', 'Birmingham', 'Manchester'], continent: 'Europe' },
        { country: 'Netherlands', code: 'NL', cities: ['Amsterdam', 'Rotterdam'], continent: 'Europe' },
        { country: 'Belgium', code: 'BE', cities: ['Brussels', 'Antwerp'], continent: 'Europe' },
        { country: 'Austria', code: 'AT', cities: ['Vienna'], continent: 'Europe' },
        { country: 'Switzerland', code: 'CH', cities: ['Geneva', 'Zurich'], continent: 'Europe' },
        { country: 'Sweden', code: 'SE', cities: ['Stockholm', 'Gothenburg'], continent: 'Europe' },
        { country: 'Poland', code: 'PL', cities: ['Warsaw'], continent: 'Europe' },
        { country: 'Russia', code: 'RU', cities: ['Moscow'], continent: 'Europe' },
        { country: 'Czech Republic', code: 'CZ', cities: ['Prague'], continent: 'Europe' },
        { country: 'Denmark', code: 'DK', cities: ['Copenhagen'], continent: 'Europe' },
        { country: 'United States', code: 'US', cities: ['Las Vegas', 'Orlando', 'New York', 'Chicago', 'Los Angeles'], continent: 'North America' },
        { country: 'Canada', code: 'CA', cities: ['Toronto', 'Vancouver', 'Montreal'], continent: 'North America' },
        { country: 'Mexico', code: 'MX', cities: ['Mexico City'], continent: 'North America' },
        { country: 'Brazil', code: 'BR', cities: ['São Paulo', 'Rio de Janeiro'], continent: 'South America' },
        { country: 'Argentina', code: 'AR', cities: ['Buenos Aires'], continent: 'South America' },
        { country: 'Colombia', code: 'CO', cities: ['Bogotá'], continent: 'South America' },
        { country: 'Australia', code: 'AU', cities: ['Sydney', 'Melbourne', 'Brisbane'], continent: 'Oceania' },
        { country: 'New Zealand', code: 'NZ', cities: ['Auckland'], continent: 'Oceania' },
        { country: 'South Africa', code: 'ZA', cities: ['Johannesburg', 'Cape Town'], continent: 'Africa' },
        { country: 'Kenya', code: 'KE', cities: ['Nairobi'], continent: 'Africa' },
        { country: 'Egypt', code: 'EG', cities: ['Cairo', 'Alexandria'], continent: 'Africa' },
        { country: 'Nigeria', code: 'NG', cities: ['Lagos'], continent: 'Africa' }
      ];

      // Check and add missing countries and cities
      for (const data of requiredData) {
        // Check if country exists
        const existingCountry = GLOBAL_EXHIBITION_DATA.countries.find(c => 
          c.name === data.country || c.countryCode === data.code
        );
        ...
        for (const cityName of data.cities) {
          const existingCity = GLOBAL_EXHIBITION_DATA.cities.find(c => 
            c.name === cityName && c.country === data.country
          );
      ...
      console.log(`✅ Location population complete: ${countriesAdded} countries, ${citiesAdded} cities added`);
  ...
  // Bulk add builders
  async addBuilders(builders: ExhibitionBuilder[], source: 'admin' | 'website' = 'admin'): Promise<any> {
    try {
