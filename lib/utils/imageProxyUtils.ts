...
export function convertToProxyUrl(url: string, baseUrl?: string): string {
  ...
  // If it's already a standszone.com URL pointing to our API, convert to relative path
  if (url.includes('standszone.com/api/media/')) {
    return url.split('standszone.com')[1];
  }
  ...
  if (match && match[1]) {
    ...
    // Default to relative URL for better compatibility with next/image
    return proxiedPath;
  }
