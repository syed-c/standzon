...
export function PublicQuoteRequest({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🚀 Submitting unified quote request via server action...', formData);
      ...
      const leadData = {
        ...
        contactPerson: formData.companyName, // Using company name as person for now
        contactPhone: formData.phone,
        tradeShow: finalExhibitionName,
        standSize: formData.standSize,
        timeline: formData.timeline,
        budget: formData.budget,
        requirements: formData.message,
        cityName: cityName,
        countryName: location || countryCodeToName[detectedCountryCode],
        status: 'Open',
        priority: formData.timeline.includes('1-2 months') ? 'High' : 'Standard',
      };

      const result = await submitLeadAction(leadData);
      ...
      setMatchingBuildersCount(result.data?.matchingBuilders || 0);
      setIsSuccess(true);

      toast({
        title: "Quote Request Sent!",
        description: `You can expect quotations from verified builders matching your criteria. They will contact you within 24 hours.`,
        duration: 5000,
      });
