...
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/shared/alert-dialog';
...
import { updateBuilderAction, deleteBuilderAction } from '@/app/actions/builders';
...
export default function ManageBuildersClient({ initialBuilders }: ManageBuildersClientProps) {
  ...
  const handleSaveBuilder = async () => {
    ...
    try {
      setSaving(true);
      console.log('Saving builder changes via server action:', editingBuilder.companyName);
      
      const result = await updateBuilderAction(editingBuilder.id, editingBuilder);
  ...
  const handleDeleteBuilder = async (builderId: string) => {
    try {
      ...
      console.log('Deleting builder via server action:', builderId);
      
      const result = await deleteBuilderAction(builderId);
  ...
  const toggleBuilderStatus = async (builder: ExhibitionBuilder, field: 'verified' | 'premiumMember') => {
    try {
      setSaving(true);
      console.log(`Toggling ${field} for builder via server action:`, builder.companyName);
      
      const updates = { [field]: !builder[field] };
      const result = await updateBuilderAction(builder.id, updates);
      ...
      if (result.success) {
        ...
        console.log(`Builder ${field} status updated!`);
      } else {
        console.error('Failed to update builder status: ' + result.error);
      }
    } catch (error) {
      console.error('Error updating builder status:', error);
      console.error('Failed to update builder status. Please try again.');
    } finally {
      setSaving(false);
    }
  };
