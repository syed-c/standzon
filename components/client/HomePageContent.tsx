...
import UltraFastHero from '@/components/client/UltraFastHero';
...
const LocationsSection = lazy(() => import("@/components/client/LocationsSection"));
const TestimonialsCarousel = lazy(() => import("@/components/client/TestimonialsCarousel"));
const ContactSection = lazy(() => import("@/components/client/ContactSection"));
const Footer = lazy(() => import("@/components/client/Footer"));

type SavedContent = any;

export default function HomePageContent({ initialContent }: { initialContent: any }) {
  const [saved, setSaved] = useState<SavedContent | null>(initialContent);
  const [isLoading, setIsLoading] = useState(!initialContent);

  // Still keep the refresh logic if needed for the editor
  const load = async () => {
    if (initialContent) return; // Skip if we have initial content
    try {
      setIsLoading(true);
      ...
      <main className="main-content">
        ...
        <section className="py-12 md:py-16 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div>
              {React.createElement(require("@/components/client/RecentLeadsSection").default, {
