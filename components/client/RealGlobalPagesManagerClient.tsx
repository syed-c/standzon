...
import LocationPageEditor from '@/components/client/LocationPageEditor';
// Remove direct Convex queries
import { generateAllGlobalPagesAction } from '@/app/actions/convex-admin';
import { 
...
export function RealGlobalPagesManagerClient({ 
  ...
  // Fetch real data from Convex via props
  ...
  // };
  const globalPagesStats = initialStats;
  ...
  const generateAllPages = async () => {
    ...
    try {
      console.log('🚀 Starting global page generation via server action...');
      
      const result = await generateAllGlobalPagesAction();
      
      if (result.success) {
        toast({
          title: "Pages Generated Successfully!",
          description: `Generated all country and city pages with real data from Convex.`,
        });
      } else {
