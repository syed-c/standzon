            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                ...
                <div className="flex gap-2 shrink-0">
                  <button className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold hover:border-[#E03A3A] transition-colors">
                    Filter by Rating
                  </button>
                  <button className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold hover:border-[#E03A3A] transition-colors">
              ...
              {builders.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center">
                  ...
                  <h3 className="text-2xl font-black text-[#252525] mb-2">No Builders Listed Yet</h3>
                  ...
                  <PublicQuoteRequest
                    ...
                    className="bg-[#E03A3A] text-white font-black px-8 py-3 h-auto rounded-lg hover:bg-[#CC2E2E] transition-all border-0"
                  />
                </div>
              )}
            </div>
      ...
      {cities && cities.length > 0 && !hideCitiesSection && !city &&
        !['jordan', 'lebanon', 'israel'].includes(country.toLowerCase()) && (
          <section className="py-20 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <h2 className="text-3xl font-black !text-[#252525] mb-12 uppercase tracking-tighter">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {cities.slice(0, 6).map((c, index) => {
                  ...
                  const rawImg = c.image || c.imageUrl || c.photo;
                  const imgSrc = isMissingImage(rawImg) ? placeholderFor('location') : rawImg;
                  const isPlaceholder = isMissingImage(rawImg);

                  return (
                    <a key={`${c.slug || c.name}-${index}`} href={cityUrl} className="group cursor-pointer">
                      <div className="h-64 rounded-2xl overflow-hidden mb-6 relative bg-[#FCFCFD] border border-[#E4E6E8]">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={imgSrc}
                          alt={isPlaceholder ? "" : `Exhibition venues in ${c.name}`}
                      ...
                      <ul className="space-y-3 text-sm font-medium !text-slate-600">
                        {features.slice(0, 3).map((feat: string, fi: number) => (
                          <li key={fi} className="flex items-center gap-2">
                            <CheckCircle className="w-4 h-4 !text-[#E03A3A] shrink-0" />
                            {feat}
                          </li>
                        ))}
                      </ul>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <h2 className="text-2xl md:text-3xl font-black !text-[#252525] mb-8 uppercase tracking-tighter">
              ...
              <article className="bg-white rounded-xl p-8 border border-slate-200">
                ...
                <div
                  className="prose max-w-none !text-slate-600 prose-p:!text-slate-600 prose-p:leading-relaxed prose-p:mb-4 prose-headings:!text-[#252525] prose-li:!text-slate-600 prose-strong:!text-[#252525] text-sm"
                  dangerouslySetInnerHTML={{ __html: servicesContent.paragraph }}
                />
                <a
                  href="#builders-grid"
                  className="!text-[#E03A3A] font-black inline-flex items-center gap-2 hover:gap-4 transition-all text-sm mt-6"
                >
            ...
            {/* Sidebar */}
            <div>
              <h2 className="text-lg font-black !text-[#252525] mb-6 uppercase tracking-tighter">
                Directory Stats
              </h2>
              <div className="space-y-3 mb-6">
                {[
                  ...
                  <div key={i} className="flex justify-between items-center p-4 bg-white border border-slate-200 rounded-lg">
                    ...
                    <span className={`text-lg font-black ${s.highlight ? "!text-emerald-600" : "!text-[#252525]"}`}>
                      {s.val}
                    </span>
                  </div>
                ))}
              ...
              <div className="p-6 bg-[#E03A3A]/10 rounded-xl border border-[#E03A3A]/20">
                ...
                <PublicQuoteRequest
                  ...
                  className="text-xs font-black uppercase !text-[#E03A3A] p-0 h-auto bg-transparent border-0 hover:bg-transparent shadow-none"
                  size="sm"
                />
              </div>
            </div>
